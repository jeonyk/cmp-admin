<!--
  * 파일명 : SetResourceEC2List.vue
  * 파일 기능 : 구성관리 > 자원관리 > EC2 리스트
  * 작성자 : 김예담
  * 최종 작성일 : 2022-01-10
...
<template>
  ...
  >
    <resource-filter-component
      ...
      @reset="() => {
        filtered.available = null
        filtered.instanceType = null
    ...
    >
      ...
      <section class="filter-form">
        ...
        <div>
          <searchable-select
            v-model="filtered.instanceType"
            :empty-text="$v('가용 영역을 먼저 선택하세요.')"
            :placeholder="$v('전체')"
            :options="typeOptions"
...
export default {
  ...
  watch: {
    searchedTags: {
      ...
      deep: true,
      handler (val) {
        this.ec2List = this.filterGridSearchedTags(val)
      }
    },
    isProjectLoaded: {
      immediate: true,
    ...
    filterGridSearchedTags (searchedTags) {
      ...
      result = this.initList.filter((el) => {
        return searchedTags.some((tags) =>
          tags.serviceType === 'EC2' && el.instanceId === tags.resourceId
        )
      })
