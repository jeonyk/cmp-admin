      ...
      >
        ...
        >
          ...
          >
            <ul>
              ...
              >
                <cmp-status-tag
                  class="__tag"
                  :contents="prj.inCommon ? this.$t('config.PROJECT.common') : $t('config.PROJECT.general')"
                  :line-style="true"
                />
                <div
                  class="__name"
          ...
          <div class="project-area__body -right">
            ...
            >
              <ul>
                <li v-if="selectedPrj.projectOwner">
                ...
                <li>{{ changeHumanTime(selectedPrj.createTime) }}</li>
              </ul>
              <!-- @AWS ACCESS KEY -->
              <form class="access-key-form">
              ...
              <div class="project-area__body__inner__history tiny-scroll">
                      ...
                      <div>
                        ...
                        >
                          ...
                          <file-down-component
                            ...
                            :file-name="selectedPrj.file.fileName"
                            @click="downloadFile(selectedPrj.file.filePath)"
                          />
                        </div>
                        <p
      ...
      <section>
        <article class="modal-body">
          ...
          >
            ...
            >
              <select-group-dropdown
                ...
                :data-source="projectModal.companyList"
                :placeholder-prefix="$t('common.TERMS.rel')"
              />
              <!-- 관계사 -->
              <select-group-dropdown
            ...
            >
              <div class="__input-pj-name">
                <el-input
                  ...
                  @keydown.native.enter="checkReduplication()"
                  class="__input"
                />
                <button
                  class="button -modal-button"
          ...
          <register-contents
            ...
            :title="$t('common.TERMS.ownerRel')"
            class="modal__possess-company"
            required
          >
            <div class="contents-wrapper">
              <template
                v-if="projectModal.ownerCompany.length > 1"
              >
                <div
                  v-for="(com, idx) in projectModal.ownerCompany"
                  :key="idx"
                  class="wrapper"
                >
                  <el-select
                    class="__select-box"
                    v-model="projectModal.ownerCompany[idx]"
                    :placeholder="$t('admin.ACCOUNT.selectAff')"
                    value-key="groupIdx"
                  >
                    <el-option
                      v-for="item in projectModal.companyList"
                      :key="item.groupIdx"
                      :value="item"
                  ...
                  <span
                    class="__delete-icon"
                    @click="deleteSelectBox(idx, projectModal.ownerCompany)"
                  />
                </div>
              </template>
              <template
              ...
              >
                <div class="wrapper">
                  <el-select
                    class="__select-box"
                    v-model="projectModal.ownerCompany[0]"
                    :placeholder="$t('admin.ACCOUNT.selectAff')"
                    value-key="groupIdx"
                  >
                    <el-option
          ...
          >
            <div class="wrapper">
              <el-upload
                ...
                :limit="1"
                ref="fileRefs"
                :file-list="projectModal.fileList"
              >
                <button
                  class="button"
                  slot="trigger"
...
<script>
import API, { CmpStatusTag, FileDownComponent, GTree, DashboardPanel, FoldCompanyGroup } from '@sd-fe/cmp-core'
import SetOwnerModal from '@/views/ConfigManage/Nutanix/SetProject/SetOwnerModal.vue'
// import FoldCompanyGroup from '@/views/ConfigManage/Nutanix/SetProject/FoldCompanyGroup'
import ChangeOpManager from '@/views/ConfigManage/Nutanix/SetProject/ChangeOpManager'
...
export default {
  ...
  computed: {
    ...
    projectModalTitle () {
      return this.showProjectModal.create ? this.$t('common.BTN.ADMIN.newProject') : this.$t('common.MODAL.modProject') // 프로젝트 생성 : 프로젝트 변경
    },
    isViewCommonOption () {
      // 최고관리자:
      // 일반 / 공통 생성 가능
  ...
  methods: {
    async getMasterkey () {
      try {
        const response = await API.aws.getMasterkey({ masterKey: 18 })
        console.log('@getMasterkey', response)
        return response
      } catch (error) {
        console.log(error)
      }
    ...
    async confirmPassword () {
      ...
      if (isTrue) {
        ...
        this.$alert('비밀번호 조회 실패.')
      }
      alert('clicked')
    },
    maskKeys () {
      this.accessKeyField = this.maskedValue
      this.secretAccessKeyField = this.maskedValue
      this.isMasked = true
    },
    searchKeys () {
      this.accessKeyField = this.accessKey
      this.secretAccessKeyField = this.secretAccessKey
      this.isMasked = false
      // this.hasAccessKeys = true
    },
    changeColor (e) {
    ...
    copyText (event) {
      ...
      const targetEle = document.getElementById(inputId)
      targetEle.select()
      navigator.clipboard.writeText(targetEle.value)
    },
    sortByASCII (a, b) {
    ...
    async getGroupManageTree () {
      try {
        ...
        if (data) this.allTreeData = this.setOrgPrjNumber(data)
    ...
    selectItem (item) {
      ...
      this.selectedCompany = `${item.groupName} (${this.$t('common.TERMS.group')} ${item.orgNum}${this.$i18n.locale === 'ko' ? '개' : ''} / ${this.$t('common.TERMS.project')} ${item.prjNum}${this.$i18n.locale === 'ko' ? '개' : ''})` // groupName (조직 *개 / 프로젝트 * 개)
      this.selectedGroupName = item.groupName
    ...
    addSelectBox () {
      const newObj = ''
      this.projectModal.ownerCompany.push(newObj)
    },
    /**
     * 소유 관계사 선택 박스 삭제
     */
    ...
    async selectOrg (org, updatePrj = false) {
      ...
      if (org) {
        ...
        let param = { groupIdx: org.groupIdx }
    ...
    selectProject (prj) {
      ...
      this.getEachProjectData(prj.projectIdx, prj.isAdmin)
    },
    /**
     * 시간 변환
     */
    ...
    validateForm () {
      ...
      if (!this.projectModal.company.idx) {
        this.$alert(this.$t('common.ALERT.PROJECT.008')) // 관계사를 선택해주세요.
        return false
      }
      if (!this.projectModal.organization.idx) {
        this.$alert(this.$t('common.ALERT.PROJECT.064')) // 조직을 선택해주세요.
        return false
      }
      if (!(this.projectModal.projectOwner && this.projectModal.projectOwner.userIdx)) {
        this.$alert(this.$t('common.ALERT.PROJECT.065')) // 프로젝트 소유자를 선택해주세요.
        return false
      }
      if (!this.projectModal.projectName) {
        this.$alert(this.$t('common.ALERT.PROJECT.063')) // 프로젝트 명을 입력해주세요.
        return false
      }
      ...
      if (this.projectModal.isCommon && !this.projectModal.ownerCompany.length) {
        this.$alert(this.$t('common.ALERT.PROJECT.067')) // 소유 관계사를 선택해주세요.
        return false
      }
      return true
    },
    ...
    async createUpdateProject () {
      const canGoNext = this.validateForm()
      if (!canGoNext) return
      ...
      let payload
      if (createMode) {
        // distGroups: [ // 공통 일 때 소유 관계사
        //   {
        //     // amount: 0, //
        //     // groups: [
        //     //   null
        //     // ],
        //     iamGroupIdx: 0, // 소유 관계사 idx
        //     name: 'string' // groupName
        //   }
        // ],
        // distributeStandardName: '', // 배부기준명
        payload = {
      ...
      try {
        ...
        if (response) {
          ...
          if (payload.inCommon) {
            // 공통 생성 시 배부 모델 페이지로 이동
            this.$router.push({ name: 'billing-model-dist' })
          } else {
            // if (createMode) {
    ...
    async checkReduplication (
      ...
      if (res) {
        ...
        this.projectModal.availablePrjName = !checkNames.includes(projectName?.trim())
      }
    },
    ...
    downloadFile (filePath) {
      ...
      this.$confirm(this.$t('common.CONFIRM.NUTA.002')) // 해당 파일을 다운로드 하시겠습니까?
        .then(() => {
        // window.location.assign(process.env.VUE_APP_ZUUL_URL + `/api/cmp/v1/nx/compute/central/images/download?userFileIdx=${userFileIdx}`)
          window.location.assign(process.env.VUE_APP_ZUUL_URL + filePath)
        }).catch(() => false)
    },
    /**
     * 사번 블라인드 처리
