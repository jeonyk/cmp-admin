...
export default {
  ...
  computed: {
    ...
    totalCount () {
      return this.vmData.length + this.vsanData.length + this.networkData.length + this.securityData.length + this.databaseData.length
    }
  },
  async created () {
    await this.init()
  ...
  methods: {
    ...
    setResourceData (all) {
      ...
      for (let i = 0; i < all.length; i++) {
        const allWorkType = all[i].service
        if (allWorkType) {
    ...
    deleteResourceFromBasket (items) {
      ...
      this.$confirm(this.$v('해당 항목을 이관 장바구니에서<br>삭제하시겠습니까?'), { dangerouslyUseHTMLString: true })
        .then(async () => {
          ...
          items.forEach(row => {
            resourceIdxArr.push(row.userVmIdx || row.userDbIdx || row.userIscsiIdx || row.csVrserverIdx || row.vrserverIdx)
    ...
    runAfterDeleteResource (items) {
      try {
        ...
        const serviceTypeArr = []
        items.forEach(row => {
          resourceIdxArr.push(row.userVmIdx || row.userDbIdx || row.userIscsiIdx || row.userShareIdx || row.vrserverIdx)
          if (!serviceTypeArr.includes(row.workType))serviceTypeArr.push(row.workType)
        })
    ...
    setRunPayload (data = this.checkedResources.all) {
      ...
      const payload = copyData.map(item => {
        ...
        return {
          ...
          resourceName: item.resourceName || item.projectName,
