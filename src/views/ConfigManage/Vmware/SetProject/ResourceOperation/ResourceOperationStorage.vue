  ...
  <div class="operation-storage">
    ...
    >
      ...
      <cmp-grid
        ...
        init-auto-select-row-key="spbmProfileUuid"
        header-checkbox
        page-keeping
      ...
      >
        ...
        <template #vmNames="props">
          <div v-if="props.row.ioOwnerHost">
            <!-- <p
        ...
        <template #hostIp="props">
          <div v-if="props.row.ioOwnerHostname">
            <!-- <p
              v-for="ip in props.row.ioOwnerHostname"
              :key="ip"
            >
              {{ ip }}
            </p> -->
            <p>{{ props.row.ioOwnerHostname }}</p>
          </div>
          <span v-else>-</span>
        </template>
        <template #vgName="props">
          <div v-if="props.row.alias">
            <p>{{ props.row.alias }}</p>
          </div>
          <span v-else>-</span>
        </template>
  ...
  methods: {
    setGridInfo (data) {
      ...
      data.forEach(row => {
        ...
        // diskSum : 크기(신청 용량)
        const diskSumBytes = this.$options.filters.volumnGroupSize(row.lunList, 'lunSize')
        row.diskSum = this.$options.filters.byte(diskSumBytes)

        // diskCount : 수량
        row.diskCount = row.lunList.length + 'EA' || 0 + 'EA'
    ...
    checkHandler () {
      ...
      const temp = uniqBy([...this.gridData, ...originCheckedData], 'userIscsiIdx')
  ...
  data () {
    return {
      ...
      columns: [
        ...
        { binding: 'vgName', header: 'Volume그룹명', keyPath: 'common.GRID.COMPUTE.volumeGroupName', align: 'left', customHtml: true },
