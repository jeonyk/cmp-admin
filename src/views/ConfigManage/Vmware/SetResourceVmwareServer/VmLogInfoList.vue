  ...
  >
    ...
    >
      <div class="filter-form">
        <span class="filter-name -required">로그 인덱스</span>
        <div class="filter-options">
          <el-select
            slot="protocol"
            v-model="filterParam.logIndex"
            placeholder="로그 인덱스 선택"
      ...
      <div class="filter-form">
        <span class="filter-name">검색일</span>
...
export default {
  ...
  methods: {
    ...
    async getElasticsearchLog (param) {
      ...
      if (!this.filterParam.logIndex) this.$alert('로그인덱스 선택은 필수입니다.')
      ...
      try {
        ...
        const list = data.hits.map((item, idx) => {
          // const itemTimestamp = item?.sourceAsMap['@timestamp'] || 0
          // this.lastLogTimestamp = this.lastLogTimestamp < itemTimestamp ? itemTimestamp : this.lastLogTimestamp

          // this.lastLogList.push(item.id)
          return {
            id: item.id,
            index: idx + 1,
