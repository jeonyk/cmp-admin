...
export default {
  ...
  methods: {
    ...
    handleApplySaveBlock (payload) {
      console.log('@handleApplySaveBlock: ', payload)
      ...
      if (this.isEditBlockModal) { // 블록 수정 적용
        ...
        this.selectedCluster.cluster = payload.cluster
      } else { // 추가
    ...
    async getIpCategories () {
      try {
        ...
        console.log('@selectedDatacenter: ', this.selectedDatacenter)
    ...
    async getElementData () {
      try {
        ...
        const rackData = await API.vmware.rack.getVmwareRackList()
    ...
    async clickTab (param) {
      ...
      // })
      this.filteredClusterList = this.clusterList
    ...
    async setTypeOptions (cateIdx) {
      ...
      await this.getClusterData()
      this.mainTypeOptions = this.clusterList
    ...
    async saveRackData (data) {
      ...
      if (action === 'confirm') {
        try {
          ...
          const response = await API.vmware.rack.changeVmwareRackList(this.rackList)
