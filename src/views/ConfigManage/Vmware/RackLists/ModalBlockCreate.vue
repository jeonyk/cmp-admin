...
export default {
  ...
  methods: {
    ...
    async setServiceHostOption (val) {
      ...
      let hostList = []
      if (val) {
        this.blockInfo.cluster = this.clusterList.filter(item => item.name === val)[0]
        const payload = {
          clusterId: this.blockInfo.cluster.clusterId,
          connectIdx: this.blockInfo.cluster.connectIdx,
          datacenterId: this.blockInfo.cluster.datacenterId
        }
        hostList = await this.getVmwareHosts(payload)
        console.log('@hostList:', payload, hostList)
      } else {
        hostList = await this.getVmwareHosts()
      }

      this.serviceHostList = hostList.map(host => {
        return {
          connectIdx: host.connectIdx,
    ...
    applySave () {
      ...
      this.$confirm(message, '알림', {
        ...
        console.log('nodeList: ', nodeList, this.blockInfo)
        const payload = {
          ...this.blockInfo,
    ...
    validForm () {
      ...
      const filteredClusters = filteredRackList.map(item => {
        console.log('filteredRackList: ', item)
        const blocks = []
        let isEditBlockName = false
        item.clusters.forEach(cluster => {
          console.log('clusterName: ', cluster.clusterName, this.blockInfo.clusterName)
          if (cluster.clusterName === this.blockInfo.clusterName && cluster.type === 'NODE') {
            console.log('blocks: ', cluster.blocks)
