<!--
  * 파일명 : UnregisterNutanixStorageList.vue
...
<template>
  <div class="unregister-nutanix-storage-list">
    ...
    <section class="table-area">
      ...
      <cmp-grid
        class="unregister-resource-grid -storage"
      ...
      >
        ...
        <template #isUrgent="{ row }">
          <span class="after-data">{{ row.afterData && row.afterData.isUrgent ? $v('긴급') : $v('일반') }}</span>
        </template>
        <!-- 분류 -->

        <template #vmNames="{ row }">
          <div v-if="row.vmNames.length">
            <p
              v-for="vm in row.vmNames"
              :key="vm"
            >
              {{ vm }}
            </p>
        ...
        <template #hostIp="{ row }">
          <div v-if="row.hostIp.length">
            <p
              v-for="ip in row.hostIp"
              :key="ip"
            >
              {{ ip }}
            </p>
...
import {
  ...
  NXStorageUpdateCoreInfoStep,
  ToggleIsUrgentCheckbox
} from '@sd-fe/cmp-core'
import { cloneDeep, isEmpty } from 'lodash'
import { mapState } from 'vuex'
import { STORAGE as requiredColumns } from '../UnregisterResourceRequiredColumns'

export default {
  name: 'UnregisterNutanixStorageList',
  components: {
    'nx-storage-core-info-step': NXStorageUpdateCoreInfoStep,
    ToggleIsUrgentCheckbox
  },
  props: {
    data: {
  ...
  computed: {
    ...mapState({
      ...
      metadataForm: state => state.metadata.NUTANIX.STORAGE
    })
  },
  watch: {
    data: {
      ...
      async handler () {
        this.setColumns()
        await this.listInfoSetting(this.data)
      }
    }
  },
  async created () {
  ...
  methods: {
    ...
    async listInfoSetting (data) {
      if (!data) {
        this.gridData = []
        return
      }

      const list = data.map(vg => {
        const {
          vms,
          disks,
          vgUuid,
        ...
        if (vms) {
          vms.forEach(vm => {
            ...
            if (vm?.nics?.length && vm?.nics[0]?.ipAddress) {
              hostIp.push(vm.nics[0].ipAddress)
        ...
        // diskSum : 크기(신청 용량)
        const diskSumBytes = this.$options.filters.volumnGroupSize(disks, 'vmDiskSizeBytes')
        const diskSum = this.$options.filters.byte(diskSumBytes) || 0
        ...
        const row = Object.assign({
          ...
          checked: this.checkedItemsId.includes(vgUuid),
    ...
    setCheckedItems (items) {
      ...
      this.checkedItemsId = items.map(item => item.vgUuid)
    ...
    filtering () {
      ...
      if (
        ...
        this.filteredData = this.gridData.filter(item => {
          ...
          if (this.filteredVgName) result = result && (item.storageName?.toLowerCase().includes(this.filteredVgName?.toLowerCase()))
          return result
        })
      }
    },
    ...
    */
    ...
    updateResource () {
      ...
      const updateItemsUuid = this.updatingItems.map(item => item.vgUuid)
        ...
        try {
          ...
          for (let i = 0; i < this.gridData.length; i++) {
            ...
            for (let j = 0; j < updateItemsUuid.length; j++) {
              ...
              if (uuid === this.gridData[i].vgUuid) {
                updatedData = {
                  ...
                  ...((isSingle || (!isSingle && !!storageName)) && { storageName }),
  ...
  data (root) {
    return {
      ...
      originColumns: [
        ...
        { binding: 'storageName', header: root.$v('Volume그룹명'), isOverflowTooltip: true, align: 'left' },
        // { binding: 'vmNames', header: root.$v('연결된 VM(Host)명'), customHtml: true, width: 180 },
        // { binding: 'hostIp', header: 'VM(Host) IP', customHtml: true },
        { binding: 'diskSum', header: root.$v('크기(신청 용량)'), width: 200 },
        { binding: 'diskCount', header: root.$v('수량'), width: 100 },
