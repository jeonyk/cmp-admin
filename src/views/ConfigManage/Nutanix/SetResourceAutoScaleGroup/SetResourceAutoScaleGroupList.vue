...
export default {
  ...
  methods: {
    ...
    async deleteASG (checkedItems = this.checkedASG) {
      this.$confirm(this.$v('선택한 오토스케일 그룹을 삭제하시겠습니까?')).then(async () => {
        try {
          const res = await API.compute.deleteAutoScaleGroupByIdx(checkedItems[0].autoScaleIdx)
          if (res) {
            this.$alert(this.$v('선택한 오토스케일 그룹이 삭제되었습니다.'), {
              callback: () => {
          ...
          if (err.code === 'COMMON0007') {
            this.$alert(this.$v('생성 진행 중인 자원이 있습니다.<br>나중에 다시 시도해주세요.'), {
              dangerouslyUseHTMLString: true
            })
          } else {
            this.$alert(this.$v('삭제 실패했습니다.'))
          }
        }
      }).catch(() => false)
    }
  ...
  data () {
    return {
      ...
      // compute 데이터
      columns: [
        { binding: 'autoScaleName', header: this.$v('오토스케일 그룹명') },
        { binding: 'ovaName', header: this.$v('사용된 VM 이미지 명') },
        { binding: 'statusKo', header: this.$v('상태') },
        { binding: 'companyName', header: this.$v('관계사') },
        { binding: 'groupName', header: this.$v('조직') },
        { binding: 'projectName', header: this.$v('프로젝트') },
        { binding: 'autoScalePolicyCnt', header: this.$v('정책 수') },
        { binding: 'vmCount', header: this.$v('최소 VM 수') },
        { binding: 'currentVmCount', header: this.$v('현재 VM 수') },
        { binding: 'startTime', header: this.$v('그룹 시작 시간'), customHtml: true },
        { binding: 'updateTime', header: this.$v('마지막 변경 시간'), customHtml: true }
      ],
      tableData: [],
      filteredData: [],
