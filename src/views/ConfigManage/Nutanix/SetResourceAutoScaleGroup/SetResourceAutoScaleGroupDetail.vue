    ...
    <section class="policy-list-section">
      ...
      <section class="table-top-wrap">
        <div class="policy-search">
          <span class="title">{{ $v('정책 상태') }}</span>
          <el-select
            v-model="filteredStatus"
            :placeholder="$v('정책 상태 선택')"
      ...
      >
        ...
        >
          <el-input
            ...
            :placeholder="$v('Auto Scale 그룹 정책 명을 입력하세요')"
...
export default {
  ...
  methods: {
      ...
      try {
        ...
        if (res?.result?.length) {
          ...
          const logs = res.result.map(obj => {
            ...
            workKo = obj.work === 'CREATE' ? '추가' : obj.work === 'DELETE' ? this.$v('삭제') : this.$v('변경')
            isSuccessKo = obj.isSuccess ? this.$v('성공') : this.$v('실패')
            if (obj.type === 'AUTO_SCALE') {
              message = this.$v('{autoScaleName} 이 {updateUserName} 에 의해 {workKo} 에 {isSuccessKo} 하였습니다.',
                { autoScaleName: this.autoScaleName, updateUserName: afterObj.updateUserName, workKo, isSuccessKo })
            }
            if (obj.type === 'AUTO_SCALE_POLICY') {
              message = this.$v('{autoScaleName} 의 {autoScalePolicyName} 이 {updateUserName} 에 의해 {workKo} 에 {isSuccessKo} 하였습니다.',
                { autoScaleName: this.autoScaleName, autoScalePolicyName: this.checkedPolicy[0].autoScalePolicyName, updateUserName: afterObj.updateUserName, workKo, isSuccessKo })
            }
            if (obj.type === 'VM') {
              const hostName = obj.work === 'CREATE' ? afterObj.hostname : beforeObj.hostname
              message = this.$v('{autoScaleName} 에 {hostName} {workKo} 에 {isSuccessKo} 하였습니다.',
                { autoScaleName: this.autoScaleName, hostName, workKo, isSuccessKo })
            }
            if (obj.type === 'VM_CREATE' || obj.type === 'VM_DELETE') {
              message = this.$v('{autoScaleName} 에 {workKo} 이(가) {isSuccessKo} 하였습니다.',
                { autoScaleName: this.autoScaleName, workKo, isSuccessKo })
            }
            if (!obj.isSuccess) {
    ...
    policyDataValidation (plcyData) {
      ...
      if (!plcyData.autoScalePolicyName || !plcyData.startDateTime || !plcyData.endDateTime) {
        this.$alert(this.$v('필수값을 입력해주세요.'))
        return false
      }
      if (startDateTime > endDateTime || startDateTime === endDateTime) {
        this.$alert(this.$v('운영 종료 일시는 운영 시작 일시보다 같거나 이전일 수 없습니다.'))
        return false
      }
      if (plcyData.vmCount <= this.groupData.vmCount) {
        this.$alert(this.$v('희망 VM 수는 그룹의 최소 VM 수 ({vmCount}개) 보다 많아야 합니다.'), { vmCount: this.groupData.vmCount })
        return false
      }
      return true
    },
    async createPolicy (e, data = this.policyInfo) {
      ...
      try {
        ...
        if (error.code === 'COMMON0010') {
          this.$alert(this.$v('운영 기간이 겹치는 정책이 있습니다.'), {
            callback: () => {
              this.loading.createPolicyLoading = false
            }
          })
        } else if (error.code === 'COMMON0006') {
          this.$alert(this.$v('그룹 내 중복된 정책명이 존재합니다.'), {
            callback: () => {
              this.loading.createPolicyLoading = false
            }
          })
        } else {
          this.$alert(this.$v('{text} 실패했습니다.', { text: '생성' }), {
            callback: () => {
              this.loading.createPolicyLoading = false
            }
          })
    ...
    async deletePolicy (e, checkedItem = this.checkedPolicy) {
      this.$confirm(this.$v('선택한 정책을 삭제하시겠습니까?'), this.$v('알림'), {
  ...
  data () {
    return {
      ...
      policyColumns: [
        { binding: 'autoScalePolicyName', header: this.$v('정책 명') },
        { binding: 'statusKo', header: this.$v('정책 상태') },
        { binding: 'startTime', header: this.$v('정책 시작'), customHtml: true },
        { binding: 'endTime', header: this.$v('정책 종료'), customHtml: true },
        { binding: 'vmCount', header: this.$v('희망 VM 수'), customHtml: true },
        { binding: 'createUser', header: this.$v('작성자') }
      ],
      vmColumns: [
        { binding: 'hostname', header: this.$v('호스트명'), customHtml: true },
        { binding: 'powerState', header: this.$v('상태'), customHtml: true },
        { binding: 'ips', header: 'IP', customHtml: true },
        { binding: 'osType', header: this.$v('OS 타입'), customHtml: true },
        { binding: 'osName', header: this.$v('OS 명'), customHtml: true },
        { binding: 'osBit', header: 'OS Bit', customHtml: true },
        { binding: 'vcpu', header: 'vCPU', customHtml: true },
        { binding: 'hypervisorCpuUsagePpmValue', header: this.$v('vCPU 사용량(%)'), customHtml: true },
        { binding: 'memory', header: 'Memory', customHtml: true },
        { binding: 'memoryUsagePpmValue', header: this.$v('Memory 사용량(%)'), customHtml: true },
      ...
      verticalTableColumns: [
        { header: this.$v('정책명'), binding: 'autoScalePolicyName', required: true },
        { header: this.$v('운영 시작 일시'), binding: 'startDateTime', required: true },
        { header: this.$v('운영 종료 일시'), binding: 'endDateTime', required: true },
        { header: this.$v('희망 VM 수'), binding: 'vmCount', required: true }
      ],
      createModal: false,
      viewLogModal: false,
      logData: [],
      ...
      // 상태 옵션
      statusOptions: [
        { value: '', label: this.$v('전체') },
        { value: 'START', label: this.$v('시작') },
        { value: 'WAIT', label: this.$v('대기') },
        { value: 'RUNNING', label: this.$v('실행 중') },
        { value: 'END', label: this.$v('종료') },
        { value: 'SUCCESS', label: this.$v('성공') },
        { value: 'FAIL', label: this.$v('실패') }
      ],
