...
<template>
  <div class="set-project">
      ...
      >
        ...
        <div class="company-area">
          ...
          >
            ...
            {{ selectedCompany ? $t('common.ALERT.PROJECT.062') : $v('관계사를 선택해주세요.') }}
          </div>
        </div>
      </dashboard-panel>
              ...
              >
                ...
                >
                  <el-tooltip
                    effect="light"
                    placement="top"
                    :disabled="!prj.message"
                  >
                    <div slot="content">
                      {{ prj.message }}
                    </div>
                    <span
                      class="tag is-waiting-deleting"
                      v-if="prj.projectStatus === 1"
                    >[{{ $v('반려') }}]
                    </span>
                  </el-tooltip>
          ...
          <div class="project-area__body -right">
            ...
            >
              <ul>
                ...
                <li>
                  {{ $v('총') }} {{ selectedPrj.inviteCount }}{{ $v('명') }}
                </li>
                <li>
                  {{ $v('총') }} {{ selectedPrj.groupShareCount }}{{ $v('개') }}
                </li>
                <li>{{ getProjectTime(selectedPrj.createTime, selectedPrj.projectStatus) }}</li>
              </ul>
              ...
              <div
                ...
                :style="{
                  height: this.historyFieldHeight
                }"
              >
    ...
    >
      ...
      <section>
        <article class="modal-body">
          ...
          >
            ...
            >
              ...
              <select-group-dropdown
                ...
                :pre-items-text="$v('관계사를 선택해주세요.')"
                :no-items-text="$t('common.ALERT.PROJECT.062')"
              /> -->

              <!-- 관계사/조직 선택 > 공통 컴포넌트(ResourceFilterComponent.vue)로 대체. (2022.08.19) -->
              <resource-filter-component
                :is-replace-filtering-component="false"
                :use-project="false"
                :use-filter-label="false"
                :init-company-idx="showProjectModal.mode === 'change' ? selectedPrj.companyIdx : undefined"
                :init-group-idx="showProjectModal.mode === 'change' ? selectedPrj.groupIdx : undefined"
                @change="selectGroup"
          ...
          >
            <p
              v-if="!projectModal.company.groupIdx"
              class="-cp-unselected"
            >
              {{ $v('관계사를 선택해주세요.') }}
            </p>
            ...
            >
              <div class="__input-pj-name">
                ...
                <button
                  ...
                  :disabled="
                    (!projectModal.projectName || !projectModal.projectName.trim()) ||
                      (showProjectModal.mode === 'change' &&
                        selectedPrj &&
                        selectedPrj.projectName === projectModal.projectName)
                  "
                >
                  {{ $v('중복 확인') }}
                </button>
          ...
          <div v-if="projectModal.isCommon && showProjectModal.mode === 'create'">
            <DistributeEditForm
          ...
          <!-- 변경 노출 -->
          <register-contents
            v-if="projectModal.isCommon && showProjectModal.mode !== 'create'"
          ...
          <register-contents
            v-if="projectModal.isCommon
              && showProjectModal.mode === 'change'
              && selectedPrj.groupIdx !== projectModal.organization.groupIdx"
    ...
    <set-owner-modal
      ...
      :title="$t('config.PROJECT.selectProjectOwner')"
...
<script>
import API, { DashboardPanel, GTree, ResourceFilterComponent, FoldCompanyGroup, DistributeEditForm } from '@sd-fe/cmp-core'
...
export default {
  ...
  computed: {
    ...
    hasNoOptions () {
      return this.allCompanyOptions.length === this.ownerCompanyInfo.ownerCompany.filter(x => x !== '').length
    },
    historyFieldHeight () {
      const { accessKey, secretKey } = this.selectedPrj
      if (this.cloudType === 'public' && accessKey && secretKey) return '270px'
      return this.cloudType === 'public' ? '310px' : '325px'
    }
  },
  methods: {
    getProjectTime (time, projectStatus) {
      const result = {
        [-1]: this.$v('승인대기'),
        2: this.changeHumanTime(time),
        1: this.$v('반려'),
        0: this.$v('승인대기'),
        3: this.$v('삭제대기'),
        4: this.$v('삭제대기')
    ...
    formatHistoryData (historyString) {
      // 프로젝트 명 변경과 소유자 변경 정보 추출하기
      const projectChanges = historyString.match(/프로젝트 명 변경: (.*) -> (.*)/)
      const ownerChanges = historyString.match(/소유자 변경: (.*) -> (.*)/)

      // 정돈된 문자열 반환
      let formattedString = ''
      if (projectChanges) {
        formattedString += '프로젝트 명 변경:\n'
        formattedString += `- 이전: ${projectChanges[1]}\n`
        formattedString += `- 현재: ${projectChanges[2]}\n\n`
      }
      if (ownerChanges) {
        formattedString += '소유자 변경:\n'
        formattedString += `- 이전: ${ownerChanges[1]}\n`
        formattedString += `- 현재: ${ownerChanges[2]}\n\n`
      }
          ...
          group.deepChildren.map(item => {
            if (item.groupProject.length > 0) {
              item.groupProject.map(pj => {
                // 관계사 출력 시 프로젝트 개수 노출
                if (!pj.isDeleted && pj.moduleType?.toLowerCase() === this.moduleType) prjNum++
              })
            }
          })
          group.prjNum = prjNum
    ...
    async selectOrg (org, updatePrj = false) {
      ...
      if (org) {
        ...
        let param = {
          ...
          moduleType: this.moduleTypes[this.moduleType]
        }
        ...
        const projectList = this.filteringIsDeleted(res)
    ...
    async projectModalOpen (mode) {
      ...
      if (mode === 'change') {
        ...
        this.projectModal = {
          ...
          // companyList: null,
          company: {
            groupIdx: this.selectedPrj.companyIdx,
            groupName: this.selectedPrj.companyName
          },
          // organizationList: null,
          organization: {
            groupIdx: this.selectedPrj.groupIdx,
            groupName: this.selectedPrj.groupName
          },
      ...
      if (mode === 'create') {
        this.showProjectModal.change = false
      } else {
        this.showProjectModal.change = true
      }
    ...
    projectModalClose () {
      ...
      if (this.showProjectModal.create && !this.showProjectModal.change) {
        console.log(12312312321321)
        console.log(this.proecj)
        console.log(this.showProjectModal)
        this.$refs.resourceComponents.resetAllFilter()
      }
    },
    /**
     * 모달창: 관-조 선택 시 실행
     */
    async selectGroup (param, mode = this.showProjectModal.mode) {
      console.log('@selectGroup', param, mode)
      // this.projectModal[mode] = { idx: param.groupIdx, name: param.groupName }
      // if (mode === 'company') { // 관계사 선택 시
      this.projectModal.company = param.company
      if (param.group) this.projectModal.organization = param.group

      if (param.company?.groupIdx === null) this.projectModal.projectName = '' // 관계사 > 전체

      if (param.company && !param.group) { // 관계사만 선택 시
        // 선택된 조직 초기화
        this.projectModal.organization = null
        // this.$refs.mdOrgDropdown.init()
        // 프로젝트 명 초기화
        ...
        try {
          ...
          // 조직 선택 리스트 세팅
          const res = await API.iam.getGroupList({ companyIdx: this.projectModal.company.groupIdx })
          ...
          // 운영 담당자 세팅
          if (this.projectModal.company?.groupIdx) await this.getOpManagerList(this.projectModal.company.groupIdx)
        } catch (err) {
    ...
    async setMidMngrAndOprMngr (groupIdx = null) {
      try {
        const grpInfo = await API.iam.getGroupList({ groupIdx: groupIdx || this.projectModal.company.groupIdx })
        if (grpInfo) {
          // 중간 관리자 태그 세팅
        ...
        this.projectModal.midMngrList = []
        this.projectModal.operatingMngrList = []
    ...
    */
     ...
     */
    async validateForm () {
      const isChangeMode = this.showProjectModal.mode === 'change'
      const isCreateMode = this.showProjectModal.mode === 'create'
      console.log(isChangeMode)
      console.log(isCreateMode)
      // console.log('@validateForm', this.projectModal)
      if (!this.projectModal?.company?.groupIdx) {
        this.$alert(this.$v('관계사를 선택해주세요.'), () => false)
        return false
      }
      if (!this.projectModal?.organization?.groupIdx) {
        this.$alert(this.$v('조직을 선택해주세요.'), () => false)
        return false
      }
      if (!(this.projectModal?.projectOwner?.userIdx)) {
        this.$alert(this.$v('프로젝트 소유자를 선택해주세요.'), () => false)
        return false
      }
      if (!this.projectModal.projectName) {
        this.$alert(this.$v('프로젝트 명을 입력하세요.'), () => false)
        return false
      }

      if (this.projectModal.operatingMngrList.some(item => item.isDeleted)) {
        this.$alert(this.$v('삭제된 담당 역할이 포함되어 있습니다.'), () => false)
        return false
      }

      // 저장 전, 삭제된 업무 역할이 포함되어 있는지 다시 확인
      if (!await this.roleExist()) {
        await this.getOpManagerList(this.projectModal.company.groupIdx)
        this.$alert(this.$v('선택한 역할이 존재하지 않습니다.<br>다시 선택해주세요.'), { dangerouslyUseHTMLString: true }, () => false)
        return false
      }

      // console.log(this.prevProjecName, this.projectModal.projectName)

      // 모드가 "생성"인데 중복 확인이 안됐거나 || 변경인데
      // if ((isCreateMode && this.projectModal.availablePrjName === null) || (isChangeMode && this.prevProjecName !== this.projectModal.projectName && this.projectModal.availablePrjName === null) || (isChangeMode && this.prevCompanyIdx !== this.projectModal.company.idx && this.projectModal.availablePrjName === null)) {
      // this.$alert(this.$t('common.ALERT.PROJECT.066')) // 프로젝트 명 중복 확인을 해주세요.

      if (
        (this.showProjectModal.mode === 'create' &&
        this.projectModal.availablePrjName === null) ||
        (this.showProjectModal.mode === 'change' &&
        this.prevProjecName !== this.projectModal.projectName &&
        this.projectModal.availablePrjName === null) ||
        (this.showProjectModal.mode === 'change' &&
        this.prevCompanyIdx !== this.projectModal.company.groupIdx &&
        this.projectModal.availablePrjName === null)
      ) {
        this.$alert(this.$v('프로젝트 명 중복 확인을 해주세요.'))
        return false
      }
      const data = this.$refs?.distributeForm?.getEmitData()
      if (this.projectModal.isCommon &&

      this.showProjectModal.mode === 'create' && !data?.distributeStandardName) {
        this.$alert(this.$v('배부 기준을 입력해주세요.'))
        return false
      }

      if (
        this.projectModal.isCommon &&
        this.showProjectModal.mode === 'create' &&
        !data?.distributeGroups.length

      ) {
        this.$alert(this.$v('1차 배부 관계사를 선택해주세요.'))
        return false
      }

      if (this.projectModal.isCommon && this.showProjectModal.mode === 'change' &&
      this.selectedPrj.groupIdx !== this.projectModal.organization.groupIdx &&

      !this.fileList?.length) {
        this.$alert(this.$v('첨부파일을 선택해주세요.')) // 프로젝트 위치 이동 시 필수 선택 요소
        return false
      }
      return true
    },
    ...
    async createUpdateProject () {
      ...
      if (createMode) {
        defaultPayload = {
          moduleType: this.getModuleType(this.moduleType),
          companyIdx: this.projectModal.company.groupIdx,
          // distGroups: this.projectModal.isCommon ? distGrops : [],
          // distributeStandardName: '',
          groupIdx: this.projectModal.organization.groupIdx,
          inCommon: this.projectModal.isCommon,
          noBilling: !this.projectModal.isBilling,
          operationManagers: mngrRoleArr, // 운영 담당자 roleIdx [5, 8]
          ownerCompany: this.projectModal.isCommon ? 1 : this.projectModal.company.groupIdx, // 공통 일 경우 1, 일반 일 경우 관계사 idx
        ...
        defaultPayload = {
          ...
          ownerCompany: this.projectModal.isCommon ? 1 : this.projectModal.company.groupIdx,
          // projectFileIdx: 0,
      ...
      if (this.projectModal.isCommon) {
        ...
        if ((this.selectedPrj?.groupIdx !== this.projectModal.organization.groupIdx) &&
        ...
        if (createMode || this.selectedPrj.commonProject) { // 배부모델이 사용자 노출Y일 때
          const commonProject = await this.setCommonProjectParams(payload)
          payload.distributeProject = commonProject
        }

        // if (!createMode) {
        //   payload.distributeProject = {}
        // }
      } else payload = defaultPayload
      console.log('@createUpdateProject: payload:', payload)

      // console.log('변경 프로젝트 ', payload)
      // const isSome = true
      // if (isSome) return
     ...
     */
    setCommonProjectParams (defaultPayload) {
      console.log('#data')
      const data = this.$refs?.distributeForm?.getEmitData()
      console.log(data)
      ...
      const result = {
        distributeGroups: data?.distributeGroups,
        distributeStandardName: data?.distributeStandardName,
        distributeModelId: data?.distributeModelId,
        projectName: defaultPayload?.projectName
        ...
        if (mngrList) {
          this.projectModal.ownerModalList = mngrList.map(man => {
            return {
              ...
              userId: man.userId,
              userName: man.userName,
              mngrType: man.managerType ? this.$v('부') : this.$v('정'),
              userCompany: this.projectModal.company.groupName,
    ...
    async checkReduplication (
      ...
      let param = { companyIdx: this.projectModal.company.groupIdx, isDeleted: false }
    ...
    updateProjectKey () {
      ...
      this.$confirm(this.$v('Access Key를 변경하시겠습니까?')).then(async () => {
        try {
          ...
          this.$alert(this.$v('Access Key를 변경했습니다.'))
    ...
    copyText (refName) {
      ...
      navigator.clipboard
        ...
        .then(() => {
          this.$alert(this.$v('복사 성공했습니다.'), () => false)
        })
        .catch(() => {
          this.$alert(this.$v('복사 실패했습니다.'), () => false)
        })
    }
  },
  data () {
