<!--
  * 파일명 : ConfigFirewallEquip.vue
  * 파일 기능 : 구성관리 > 방화벽 장비 정보
  * 작성자 : 이유준
  * 최종 작성일 : 2021-01-26
      ...
      <div class="button-area -right">
        <div class="button-area">
          <button
            class="button"
            type="is-primary"
            @click="e => handleOpenFirewallEdit('CREATE')"
          >
            {{ $v('항목 추가') }}
          </button>
          <button
            ...
            @click="e => handleOpenFirewallEdit('UPDATE')"
          >
            {{ $v('변경') }}
          </button>
          <button
            class="button"
            type="is-anti"
            @click="handleClickFirewallDelete(selectedRow)"
            :disabled="selectedRow === null"
          >
            {{ $v('삭제') }}
          </button>
    ...
    <section class="table-area">
      ...
      >
        ...
        <template #detail="props">
          <a
            @click.self="handleClickFirewallDetail(props.row.netIdx)"
            class="-link"
          >더보기</a>
        </template>
...
import ModalFirewallEquipEdit from './ModalFirewallEquipEdit.vue'

export default {
  name: 'ConfigFirewallEquip',
  components: {
    CmpStatusTag,
    ModalFirewallEquipEdit
  },
  computed: {
    ...mapState({
  ...
  methods: {
    ...
    **/
    handleOpenFirewallEdit (type) {
      this.modalStatus.type = type
      this.modalStatus.isOpen = true
    },
    // EVENT_HANDLER (선택) 그리드 로우 선택
    handleSelectedRow (sRow) {
      console.log('🚀 ~ file: ConfigFirewallEquip.vue:154 ~ handleSelectedRow ~ sRow:', sRow)
      if (sRow?._data) { this.selectedRow = sRow._data } else this.selectedRow = null
    },
    async initTable () {
      this.tableData = await this.getEquipFirewallList()
    },
    async getEquipFirewallList () {
      this.loading = true
      try {
        const response = await API.network.getEquipFirewallList()
        return response
      } catch (error) {
        console.error(error)
        this.tableData = []
     ...
     */
    async handleClickFirewallDelete (removeObj) {
    ...
    /** EVENT_HANDLER  */
    async handleClickFirewallDetail (netIdx) {
      this.$router.push({ name: 'firewall-equip-detail', params: { netIdx: netIdx } })
    }
  },
  data () {
    return {
      ...
      // 테이블 정보
      columns: [
        { header: this.$v('방화벽 장비명'), binding: 'netName', width: 250, align: 'left' },
        ...
        { header: this.$v('상세'), binding: 'detail', width: 100, customHtml: true }
      ],
      MOCK_DATA: [{
        id: 'testUser1',
        netIdx: 2,
        ...
        apiUrl1: 'http://localhost:8080/main/link-setting/nx/firewall-equip',
        apiUrl2: 'http://localhost:8080/main/link-setting/nx/firewall-equip',
        masterUrl: 'http://localhost:8080/main/link-setting/nx/firewall-equip',
        intervalSec: 3,
        isManualManagement: true,
        comment: 'Firewall 장비가 하드코딩되어 있습니다.',
