        ...
        <template v-if="updateForm.alarmType !== ''">
          <div>
            ...
            <register-contents
              ...
              v-if="updateForm.alarmType === 'PPURIO_AT' && updateForm.alarmTemplate.length>0"
              required
            >
              <div
...
export default {
  ...
  methods: {
    callbackApi: API.alarm.getAlarmInfoCert,
    validatedOTP (data) {
      this.otp = true
    ...
    async getAlarmTypeOptions () {
      try {
        ...
        const alarmTypes = await API.config.getCodeList({ codeType: 'ALARM_TYPE' })
    ...
    async getAlarmInfo () {
      try {
        this.loading = true
        const response = await API.alarm.getAlarmInfo()
        if (!response) return
    ...
    deleteAlarmInfo () {
      ...
      this.$confirm(this.$t('common.CONFIRM.BASE.032'), '삭제', {
        ...
        const result = await API.alarm.deleteAlarmInfo(alarmType)
    ...
    async createAlarmInfo (updateRow) {
      const result = await API.alarm.createAlarmInfo(updateRow)
    ...
    async updateAlarmInfo (alarmType, alarmInfo) {
      const result = await API.alarm.updateAlarmInfo(alarmType, alarmInfo)
    ...
    updateValid () {
      ...
      if ((this.updateForm.alarmType === 'PPURIO_AT' || this.updateForm.alarmType === 'PPURIO_SMS') &&
          !this.updateForm.sendType) {
        this.$alert(this.$t('전송 타입을 입력해주세요')) // 전송 타입을 유효성 검사
        return false
      }
      ...
      if (this.updateForm.alarmType === 'PPURIO_AT') { // 뿌리오 알림톡 유효성 검사
        for (let i = 0; i < this.updateForm.alarmTemplate.length; i++) {
          if (!this.updateForm.senderId) {
            this.$alert(this.$t('발신 번호를 입력해주세요.'))
            return false
          }
          if (!this.updateForm.systemKey) {
            this.$alert(this.$t('발신 프로필 키를 입력해주세요.'))
            return false
          }
          if ((this.updateForm.sendType === 'API' && !this.updateForm.alarmTemplate[i].apiTemplateCode) ||
              (this.updateForm.sendType === 'DB' && !this.updateForm.alarmTemplate[i].dbTemplateCode)) {
            this.$alert(this.$t('템플릿 코드를 입력해주세요.'))
            return false
          }
          if (!this.updateForm.alarmTemplate[i].nationCode) {
            this.$alert(this.$t('국가 코드를 입력해주세요.'))
            return false
          }
          if (!this.updateForm.alarmTemplate[i].templateText) {
            this.$alert(this.$t('템플릿 텍스트를 입력해주세요.'))
            return false
          }
        }
      }
      if (this.updateForm.alarmType === 'PPURIO_SMS' && !this.updateForm.senderId) {
        this.$alert(this.$t('발신 번호를 입력해주세요.'))
        return false
      }
      return true
