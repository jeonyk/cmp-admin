...
export default {
  ...
  methods: {
    ...
    async getMeteringStatus (params) {
      try {
        ...
        const response = await API.metering.getMeteringStatus(params ? { ...params, moduleType: this.moduleType } : { moduleType: this.moduleType })
        if (!response) return
    ...
    setServerTableData (data) {
      ...
      const tableData = data.map(item => {
        // 설치프로그램
        item.swCount = '-'
        if (item.useSwList?.length) {
          for (let i = 0; i < item.useSwList.length; i++) {
            item.swCount = item.useSwList[i]?.name
          }
        }
        // Storage 합계
  ...
  watch: {
    startDate (newStartDate, oldDate) {
      if (this.endDate && new Date(newStartDate) > new Date(this.endDate)) {
        this.startDate = oldDate
        return this.$alert('시작일은 종료일보다 클 수 없습니다.')
      }
      if (newStartDate !== oldDate) this.filteringWithDate(newStartDate, this.endDate)
    },
    endDate (newEndDate, oldDate) {
      if (this.startDate && new Date(newEndDate) < new Date(this.startDate)) {
        return this.$alert('종료일은 시작일보다 작을 수 없습니다.')
      }
      ...
      display: flex;
        > .body-panel {
          ...
          &.-mid {
            ...
            .table-area #cmpGrid .wj-cell > div{
              white-space: nowrap;
            }
          }
