    ...
    <section class="detail-wrapper">
      <article>
        <div class="sw-version-info">
          ...
          <button
            class="button"
            type="is-primary"
            @click="updateProgram('versionAdd', `S/W ${$t('common.BTN.add')}`)"
            :disabled="installableVersionCount === 0"
          >
            <!-- SW 버전 추가 -->
            {{ $t('service.INSTALL.swVersionAdd') }}
          </button>
        </div>
        <!-- /. SW 정보, 버전 추가  -->
      ...
      <article v-if="selectedVersion !== null">
        ...
        <div class="detail-description-wrapper">
            ...
            <div class="version-desc-buttons">
              ...
              <div>
                <!-- <button
                  class="button"
                  type="is-primary"
                  @click="$router.push({ name: 'install-program-detail-install', params: { versionId: selectedVersion.id } })"
                >
                  Install to VM
                  => (220404) 기획이 잘못되어 페이지가 삭제되었습니다
                </button> -->
                <button
                  class="button"
                  type="is-primary"
                  @click="$router.push({ name: 'install-program-detail-installed', params: { versionId: selectedVersion.id } })"
                >
                  Installed VM List
                </button>
              </div>
          ...
          <div class="install-files-wrapper">
            <div class="install-files flex-wrap -space-between">
              <h5 class="-title">
                <!-- 설치 파일 관리 -->
                {{ $t('service.INSTALL.manageInstallFile') }}
              </h5>
              <button
                class="button"
                type="is-primary"
                @click="updateFile('add', $t('service.INSTALL.addFile'))"
              >
                <!-- 파일 추가 -->
                {{ $t('service.INSTALL.addFile') }}
              </button>
            </div>
            ...
            <cmp-grid
              style="width: 1260px"
              :paging-size="5"
              :item-source="installFileData"
              :columns="installFileColumn"
            ...
            >
              <template #fn="props">
                <div class="function-icons">
                  <i
                    class="-edit-icon"
                    @click="updateFile('update', $t('service.INSTALL.updateFile'), props.row)"
                  />
                  <i
                    class="-delete-icon"
...
export default {
  ...
  watch: {
    selectedVersion (version) {
      ...
      this.installFileData = this.setInstallFileData(version)
  ...
  methods: {
    ...
    async setVersionsFormat (data) {
      return data.map(({ version, bitType, osType, status, description, ...item }) => {
        return {
          ...item,
          version,
          osType,
     ...
     */
    updateSw ({
      swIdx, supportOs, osBits, swVersion, versionId, softwareId, desc, thumbnailImage, thumbnailName,
      script, scriptId, versionDesc,
      ...items
    }) {
      const payload = {
        id: versionId,
        ...
        softwareId,
        version: swVersion,
        description: versionDesc, // version 의 설명
        thumbnailName,
        thumbnailImage, // base64 encode
        updateUserIdx: this.user.userIdx
      }

      // 스크립트가 선택된 상황이라면 변경된 스크립트에 대한 정보 추가
      if (script !== undefined) {
        payload.scriptList = [
          {
            description: desc, // 해당 버전 > script > 설명 변경
            id: scriptId, // script version
      ...
      // console.log(JSON.stringify(payload))
      return this.updateScript(payload)
    },
