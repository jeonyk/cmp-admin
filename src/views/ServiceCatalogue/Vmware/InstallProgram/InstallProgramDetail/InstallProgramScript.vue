  ...
  <section class="install-program-script-wrapper">
    ...
    >
      ...
      <div class="software-version-information">
        <strong>
          <!-- 스크립트 버전 정보 -->
          {{ $t('service.INSTALL.ScriptVerInfo') }}
          <span>*</span></strong>
        <el-input
          class="-input"
      ...
      <el-input
        ...
        :placeholder="$t('service.INSTALL.PLACEHOLDER.008')"
      />
      <!-- /. 스크립트 설명 -->
...
export default {
  ...
  methods: {
    ...
    async getTestVMList () {
      try {
        const response = await API.sw.getTestVMList({ osType: this.version.osType, bitType: `X${this.version.bitType}` })
        // console.log('test vms :: ', response)
        this.testVMOptions = response.map(({ hostName, vmUuid, hostUuid, osType, id, ...vm }) => ({ label: hostName, key: vmUuid, value: vmUuid, data: { hostUuid, osType, hostName, id } }))
      } catch (error) {
    ...
    setTestPayload (tabIdx) {
      ...
      const payload = {
        ...
        softwareMinionList: [this.selectedTestVm[tabIdx]]
      }
    ...
    */
    ...
    async applyVersion () {
      ...
      const { scriptVersion, isUse, description } = this.scriptInfo
      ...
      if (result.status) {
        ...
        const code = {
          SWM3003: this.$t('service.INSTALL.ALERT.020') // 이미 해당 버전이 존재합니다
        }
        // 저장에 실패했습니다 {code 정보}
        this.$alert(this.$t('service.INSTALL.ALERT.021') + `${result.code} : ${code[result.code]}`, '', {
  ...
  data () {
    return {
      ...
      selectedTestVm: {}, // VM Run 시 필요한 softwareMinionList 를 저장합니다
