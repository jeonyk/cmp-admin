    ...
    <div v-loading="loading">
      <section class="modal-body add-sw">
        <article class="sw-lists">
          ...
          >
            <strong class="title">
              {{ item.title }} <b>{{ item.title ? '*': null }}</b>
            </strong>
            <el-select
              class="-input"
        ...
        <article class="division">
          <!-- 썸네일 이미지 -->
          <strong class="title">{{ $t('service.INSTALL.thumb') }}</strong>

          <el-upload
            action=""
            class="cmp-upload -absolute"
            :multiple="false"
            :show-file-list="true"
            :auto-upload="false"
            :file-list="fileList"
        ...
        <div class="division">
          <strong class="title">{{ $t('service.INSTALL.swDesc') }}</strong>
          <!-- 설명을 입력해주세요 -->
          <el-input
