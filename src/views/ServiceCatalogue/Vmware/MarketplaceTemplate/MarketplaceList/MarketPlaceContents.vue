      ...
      <ul class="marketplace-wrap">
        ...
        >
          ...
          <div class="list-info">
            ...
            <div class="-sv-detail">
              <small>[Ver] {{ osType.pgVersion }} ({{ osType.updateTime | date }})</small>
              <small>[OS] {{ osType.osInfo }}</small>
              <div class="flex-wrap -sw-license">
                <span style="white-space: nowrap; margin-top: 3px;">[{{ $t('service.sw') }}]</span>
  ...
  methods: {
    ...
    getBpDescInfo () {
      this.osTypeData = this.osTypeData.map(os => {
        let osInfo = null
        if (os.bpImg && os.bpImg.osType) osInfo = `${os.bpImg?.osType} ${os.bpImg?.osName} ${os.bpImg?.osBit}`
        const result = { ...os }
    ...
    async getSwLicenseList () {
      const installationSwLicenseList = await API.config.getSwLicenseList({ category: 'installation', isDeleted: true })
      const osSwLicenseList = await API.config.getSwLicenseList({ category: 'image', isDeleted: true })
