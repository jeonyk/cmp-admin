      ...
      <template #distCompany_2>
        ...
        >
          <div class="common-name-wrap">
            <span class="common-name">{{ common.commonAffName + "(배부기준 : " +(common.distributeStandardName || '고정 값')+ ")" }}</span>
            <a
              class="-delete-icon"
...
export default {
  ...
  data () {
    return {
      ...
      ownerCompanyColumns: [
        { binding: 'distModelName', header: '배부모델명', customHtml: true, required: true },
        { binding: 'distName', header: '배부기준명', customHtml: true, required: true },
        { binding: 'distCompany_1', header: '1차 배부 관계사', customHtml: true, required: true },
        { binding: 'distCompany_2', header: '2차 배부 관계사', subTitle: '(1차 배부 관계사 중 선택 가능)', customHtml: true }
      ]
    }
  }
}
