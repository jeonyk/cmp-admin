...
export default {
  ...
  methods: {
    resetFilters () {
      ...
      this.selectedDate.start = Dayjs().subtract(6, 'month').format('YYYY-MM-DD')
      this.selectedDate.end = Dayjs().add(6, 'month').format('YYYY-MM-DD')
    },
    filterByDate (item) {
      const expectDate = Dayjs(item.expectDate || item.expectedDate)
      const { start, end } = this.selectedDate
      return expectDate >= new Dayjs(start) && expectDate <= Dayjs(end)
    },
    newFilter (options) {
      console.log('#newFilter')
      const filter = this.$refs.filterComponent
      const { start, end } = this.selectedDate
      let result = [...this.rawData]
      if (filter?.filteredCompany) result = result.filter(x => x.companyIdx === filter.filteredCompany)
      if (filter?.filteredGroup) result = result.filter(x => x.groupIdx === filter.filteredGroup)
      if (filter?.filteredProject) result = result.filter(x => x.projectIdx === filter.filteredProject)
      if (this.selectedOrderType) result = result.filter(x => x.orderTypeKor === this.selectedOrderType)
      if (start && end) result = result.filter(this.filterByDate)
    ...
    filterHandler () {
      return this.setFilters(this.selectedFilter)
    },
    /**
    ...
    setFilters (filter = this.selectedFilter, rel) {
      let result = [...this.rawData]
      if (filter?.company) result = result.filter(x => x.ordererCompany.idx === filter.company)
      if (filter?.workState) result = result.filter(x => x.workState === filter.workState)
      if (filter?.orderTypeKor !== undefined) {
        result = result.filter(x => x.orderTypeKor === filter.orderTypeKor)
      }
      if (filter?.title) {
        result = result.filter(x => x.orderNo.includes(filter.title))
      }
      this.taskData = result
      this.$forceUpdate()
    },
