...
export default {
  ...
  methods: {
    ...
    async intervalFunction () {
      ...
      let workResult = null
      let response = await API.work.getTaskTodo(this.orderNo)
      // 자동/수동인지 확인
    ...
    async setTaskTodoInfo (params) {
      try {
        const response = await API.work.getTaskTodo(params)
    ...
    async getTaskResult () {
      try {
        return await API.work.getTaskResult(this.orderNo)
      } catch (error) {
        console.error(error)
      }
    },
    ...
    async updateTaskTodoChkList (chkParam, roleInfo) {
      ...
      const res = await API.work.updateTaskTodoChkList(payload)
      if (res) {
