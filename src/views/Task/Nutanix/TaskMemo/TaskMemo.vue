  ...
  <div :class="['temp', { '-mini-temp': size === 'mini' }]">
    <div :class="size === 'mini' ? '-mini-wrap' : ''">
        ...
        >
          ...
          >
            <div class="indiv-comment">
              ...
              <div
                class="more-action"
                v-if="!isSystemMode && ((!disableWriting && removable(comment)) && !comment.isAuto) || size === 'mini'"
              >
                <i
      ...
      <div
        ...
        v-if="!isSystemMode && (!openWriter && size !== 'mini' && !disableWriting)"
      >
        <button
          class="button"
          @click="addMemo('write')"
...
export default {
  ...
  methods: {
    ...
    async removeMemo (idx) {
      try {
        const deleteMemo = await API.work.deleteMemo(idx)
