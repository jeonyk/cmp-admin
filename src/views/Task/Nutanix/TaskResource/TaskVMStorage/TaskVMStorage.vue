  ...
  <div class="task-compute task-resource-grid">
    ...
    >
      <div class="process-top-area">
        <span class="sub-title">vSAN iSCSI</span>
    ...
    >
      <template #networkList="props">
        ...
        >
          {{ props.row.networkList[0].cateKey }}
        </div>
      </template>
      <!-- /. 네트워크 -->

      <template #lunList="props">
        <div
          class="cell-flex-wrap"
          v-if="props.row.lunList"
        >
          {{ props.row.lunList[0].alias }}
        </div>
      </template>
      <!-- /. LUN -->
...
import TaskSourceModalVMwareVsan from '@/components/Modal/TaskSourceModal/TaskSourceModalVMwareVsan'
...
export default {
  ...
  components: {
    ...
    'task-source-modal-vsan': TaskSourceModalVMwareVsan,
  ...
  created () {
    this.columns = this.setColumns_V3(this.rawColumns, 'VSAN_ISCSI')
