  ...
  <div class="task-compute task-resource-grid">
    ...
    >
      <div class="process-top-area">
        <span class="sub-title">File Server Share</span>
...
import TaskSourceModalFileServer from '@/components/Modal/TaskSourceModal/TaskSourceModalFileServer'
...
export default {
  ...
  components: {
    ...
    'task-source-modal-file-server': TaskSourceModalFileServer,
  ...
  created () {
    this.columns = this.setColumns_V3(this.rawColumns, 'FILE_SERVER')
  ...
  methods: {
    ...
    setInitData (tableData) {
      ...
      const list = cloneData.map(row => {
        ...
        return {
          ...
          scheduling, // 예약설정
          displayMaxGiBSize: row.maxSizeGiB + ' GB' // 읽기전용
    ...
    async selectSourceInfo (data) {
      ...
      this.shareType = `FILE_SERVER_${data.row?.protocolType}`
    }
  },
  data () {
    return {
      ...
      shareType: undefined, // 프로토콜 타입에 따라 다르게 바인딩
