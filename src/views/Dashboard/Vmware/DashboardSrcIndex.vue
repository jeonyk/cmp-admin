      ...
      const clusterInfos = rawClusters.map(cls => {
        ...
        const nodes = hosts?.map(host => {
          ...
          const assignPct = {
            ...
            cpu: { value: cpuUsagePct, ...cpuExceeded },
            memory: { value: memUsagePct, ...memoryExceeded },
            disk: { value: 0 } // 노드에는 disk 할당량이 없음..!!
          }
        ...
        const assignPct = {
          ...
          cpu: { rank: vcpuRank, value: cpuUsagePct, ...cpuExceeded },
          memory: { rank: memoryRank, value: memUsagePct, ...memoryExceeded },
          disk: { value: diskCapacityPct, ...diskExceeded }
        }
