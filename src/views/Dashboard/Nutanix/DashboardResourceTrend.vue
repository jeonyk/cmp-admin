...
import ConvertLangUtils from '@/components/Utils/ConvertLangUtils'
  ...
  methods: {
    ...
    setTrend (data) {
      ...
      for (const d of data) {
        ...
        const dataObj = {
          date: this.convertMonth(new Date(date).getMonth() + 1),
          clusterName,
          clusterUuid,
          region
        }
        ...
        for (var i = 0; i < 10; i++) {
          ...
          if (d.diskTotSize > 0 && d.diskTotSize < 1024) {
            diskObj = {
              date: this.convertMonth(new Date(d.date).getMonth() + 1),
              all: d.diskTotSize,
              usage: d.diskUseSize,
              clusterName,
              clusterUuid,
