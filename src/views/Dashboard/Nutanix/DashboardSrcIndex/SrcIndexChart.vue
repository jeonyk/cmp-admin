  ...
  >
    ...
    <div class="nutanix-alert">
      <div
        ...
        :key="key"
        :class="['alert-content', { '-pointer': addPointer(key) }]"
        @click="addPointer(key) ? routeToConfigServer() : null"
      >
      ...
      <ul class="rate-chart">
        ...
        >
          <div class="graph">
            ...
            >
              <!-- 관리지표 :: 최대임계치 -->
              <small>{{ $v('관리지표') }}: {{ item.indicator }}</small>
...
export default {
  ...
  methods: {
    checkType (data) {
      this.isCluster = data.type === 'cluster'
    },
    async getAlertCnt (data) {
      ...
      const contents = this.isCluster ? { clsNodeCnt: children.length - 1, clsVmCnt } : { nodeVmCount: vmCnt, nodeRunVM: vmRun }
      ...
      try {
        ...
        if (this.isCluster) payload = { elementIdx: data.elementIdx }
    ...
    getAssignPercent (data) {
      ...
      this.virtualizeRate = this.virtualizeRate.map(item => {
        ...
        const { value, indicator, standard1, standard2 } = assignPct
        ...
        // value: 데이터 , percent: 그래프 데이터, indicator: 관리지표, max: 임계치, spareRank: 여유 n 순위, status: 임계치 over 여부
        return { ...item, value, percent, indicator, max, spareRank, status }
      })
    },
    /**
     * [VM Count] 클릭시 페이지 [구성관리 > Compute] 로 이동합니다
     */
    routeToConfigServer (data = this.data) {
      let query
      if (this.isCluster) query = { clusterUuid: data.uuid }
      else query = { clusterUuid: data?.clusterInfo.uuid, hostUuid: data.uuid }

      return this.$router.push({ name: 'set-resource-server-list', query })
    }
  },
  data () {
    return {
      ...
      addPointer (key) { // VM Count 인지 확인
        return /clsVmCnt|nodeVmCount/gi.test(key)
      }
    }
  }
}
  ...
  .nutanix-alert {
    ...
    .alert-content {
      ...
      &.-pointer {
        ...
        &:hover::before {
          border-bottom: 2px solid rgba(93, 103, 255, 0.3);
        }
      }
    }
  }
