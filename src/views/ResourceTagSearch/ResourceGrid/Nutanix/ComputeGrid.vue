<template>
  <section class="table-area">
    <cmp-grid
      v-loading="!this.interval ? loading.isGetVmRequest : isRefreshGrid"
      ref="serverGrid"
...
export default {
  ...
  methods: {
     ...
     */
    filtering () {
      if (!this.filteredVmState && !this.filteredHostName && !this.filteredCluster && !this.filteredHostName) this.filteredData = this.tableData
      else {
  ...
  data () {
    return {
      ...
      columns: [
        ...
        { binding: 'powerState', header: '상태', width: 100, customHtml: true, keyPath: 'common.GRID.state' },
