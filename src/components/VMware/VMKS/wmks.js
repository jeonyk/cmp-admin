...
(function () {
  ...
  function d(a) {
    ...
    var b = a,
      ...
      g = function (a) {
        ...
        e.push32(Date.now()), a.src = 'data:image/gif;base64,' + c.encodeFromArray(e), d.length <= b ? d.push(a) : f([a])
      };
  ...
  WMKS = {}, WMKS.LOGGER = new function () {
    ...
    createCanvas: function (a) {
      var b = {};
      return a && (b.position = 'absolute'), document.querySelector('<canvas/>').css(b)
    },
    createVideo: function (a) {
      var b = {};
      return a && (b.position = 'absolute'), document.querySelector('<video/>').css(b)
    },
    ...
    function () {
      ...
      }), this.setRenderCanvas(this.options.canvas), this.options.backCanvas && (this._canvas = this._canvas.concat([this.options.backCanvas]), this._canvas[1].ctx = this.options.backCanvas.getContextContent='2d')), this.options.blitTempCanvas && (this._canvas = this._canvas.concat([this.options.blitTempCanvas]), this._canvas[2].ctx = this.options.blitTempCanvas.getContextContent='2d')), this.options.mediaPlayer && (this._mp4Decoder = new e), typeof createImageBitmap != 'undefined' ? (this._useImageBitmaps = !0, this._imageManager = null) : this._imageManager = new d(256), this._releaseImage = function (a) {
      ...
      var i;
      i = this._canvas[a].ctx.getImageData(b, c, d, e), this._canvas[f].ctx.putImageData(i, g, h), i = null
      ...
      this._canvas[0] = a, this._canvas[0].ctx = a.getContextContent='2d');
      ...
      this._vncDecoder = a.vncDecoder, this.ignoredRawKeyCodes = a.ignoredRawKeyCodes, this.fixANSIEquivalentKeys = a.fixANSIEquivalentKeys, this.mapMetaToCtrlForKeys = a.mapMetaToCtrlForKeys, this.keyDownKeyTimer = null, this.keyDownIdentifier = null, this.pendingKey = null, this.addCtrlKey = !1, this.activeModifiers = [], this.keyToUnicodeMap = {}, this.keyToRawMap = {}, this.keyboardLayoutId = a.keyboardLayoutId, this.UnicodeToVScanMap = WMKS.CONST.KB.VScanMap[this.keyboardLayoutId], this._windowsKeyManager = {
        ...
        var b = a.keyCode || a.which;
        return WMKS.BROWSER.isAndroid() && WMKS.BROWSER.isChrome() ? !0 : (document.querySelector(a.target).val(WMKS.CONST.KB.keyInputDefaultValue), this.handleSoftKb(b, !0), !0)
      }, this.onInputTextSoftKb = function (a) {
        var b = document.querySelector(a.target),
          c = b.value,
      ...
      this._verifyQuickTouches = function (a, b, c) {
        ...
        var b, c, d, e, f, g = !1;
        return WMKS.BROWSER.isTouchDevice() ? !a || a.matches(':hidden') ? !1 : (b = a.getBoundingClientRect().width, c = a.getBoundingClientRect().height, e = window.innerWidth, f = window.innerHeight, WMKS.UTIL.TOUCH.isPortraitOrientation() ? a.offset().left + b > e && (a.offset({
          left: String(e - b - 5)
        ...
        var b = this,
          c = f.offset();
        this.widgetRepositionOnRotation(j.inputProxy), this.widgetRepositionOnRotation(j.cursorIcon), j.clickFeedback.offset(c), j.dragFeedback.offset(c), j.pulseFeedback.offset(c), j.scrollFeedback.offset(c), $.forEach(e, function (a, c) {
          try {
        ...
        var b = this;
        this._isInputInFocus() && document.querySelector(window).one('resize', function (a) {
          setTimeout(function () {
            document.querySelector(window).trigger('orientationchange'), b._repositionFloatingElementsOnRotation()
          }, 500)
        })
        ...
        e = g.offsetLeft || 0, f = g.offsetTop || 0, d = WMKS.UTIL.TOUCH.getRelativePositionMultiplier(g.position), a.css({
          left: b.pageX + e + a.offsetWidth * d.width,
          top: b.pageY + f + a.offsetHeight * d.height
        }), this.moveCursor(b.pageX, b.pageY), a.classList.remove('animate-feedback-indicator animate-double-feedback-indicator'), g.showTwice ? setTimeout(function () {
          a.classList.add('animate-double-feedback-indicator')
        }, 0) : setTimeout(function () {
          a.classList.add('animate-feedback-indicator')
        }, 0)
        ...
        j.cursorIcon && (a ? j.cursorIcon.style.display = '' : j.cursorIcon.style.display = 'none')
        ...
        a ? (f.focus(), j.inputProxy.focus().select()) : (WMKS.BROWSER.isAndroid() && (j.inputProxy.getAttribute('readonly', !0).getAttribute('disabled', !0), setTimeout(function () {
          j && (j.inputProxy.getAttribute('readonly', !1).getAttribute('disabled', !1), f.focus())
        ...
        d.visible = a, d.lastToggleTime = Date.now(), $.isFunction(g) && g.call(this, ['KEYBOARD', d.visible])
        ...
        if (Date.now() - d.lastToggleTime < WMKS.CONST.TOUCH.minKeyboardToggleTime) {
        ...
        }), j.cursorIcon = document.querySelector('<div/>').classList.add('feedback-container cursor-icon').append(b), j.clickFeedback = document.querySelector('<div/>').classList.add('feedback-container tap-icon').append(b), j.dragFeedback = document.querySelector('<div/>').classList.add('feedback-container drag-icon').append(b), j.pulseFeedback = document.querySelector('<div/>').classList.add('feedback-container pulse-icon').append(b), j.scrollFeedback = document.querySelector('<div/>').classList.add('feedback-container scroll-icon').append(b), b.find('.feedback-container').bind('touchmove.wmks', function (b) {
          return a._onTouchMove(b.originalEvent)
        }).bind('touchstart.wmks', function (b) {
          return a._onTouchStart(b.originalEvent)
        }).bind('touchend.wmks', function (b) {
        ...
        var a = this,
          b = c,
          d = document.querySelector('<input type='text'/>').val(WMKS.CONST.KB.keyInputDefaultValue).getAttribute({
            id: 'input-proxy',
            autocorrect: 'off',
            autocapitalize: 'off'
          }).css({
      ...
      return this.options.useNativePixels ? (e = Math.ceil(this._canvas.getBoundingClientRect().width * d) - 1, f = Math.ceil(this._canvas.getBoundingClientRect().height * d) - 1) : (e = Math.ceil(this._canvas.getBoundingClientRect().width) - 1, f = Math.ceil(this._canvas.getBoundingClientRect().height) - 1), g = Math.min(g, e), h = Math.min(h, f), g = Math.max(g, 0), h = Math.max(h, 0), {
        x: g,
        y: h
      }
      ...
      b = this._canvas.offset(), c = this._canvas.parentNode.offset(), d = this._scale / this._pixelRatio;
      ...
      return this._mouseDownBMask !== 0 ? !0 : c === this._canvas[0] || this._video && c === this._video[0] || c === document.querySelector('#relativePadSurface')
      ...
      }), document.querySelector(window).bind('blur.wmks', function (b) {
        return a._onBlur(b)
      ...
      var b = this.element.getBoundingClientRect().width * this._pixelRatio,
        c = this.element.getBoundingClientRect().height * this._pixelRatio;
      ...
      var b = 1,
        c = 0,
        d = 0,
        e = this.element.getBoundingClientRect().width,
        f = this.element.getBoundingClientRect().height;
      this._canvas.css({
        width: this._guestWidth / this._pixelRatio,
        height: this._guestHeight / this._pixelRatio
      });
      var g = this._canvas.getBoundingClientRect().width,
        h = this._canvas.getBoundingClientRect().height;
      ...
      this.disconnect(), this.element.classList.remove('wmks'), this._touchHandler.destroy(), this._touchHandler = null, this._relativeMouseHandler.destroy(), this._relativeMouseHandler = null, this._canvas.remove(), this._video && this._video.remove(), this._backCanvas && this._backCanvas.remove(), this._blitTempCanvas && this._blitTempCanvas.remove(), $.Widget.prototype.destroy.call(this)
      ...
      this._trigger('useractivity', 0, Date.now())
    }, WMKS.widgetProto._create = function () {
      var a = this;
      this._mouseDownBMask = 0, this._mousePosGuest = {
        x: 0,
        y: 0
      }, this._scale = 1, this._pixelRatio = 1, this.connected = !1, this.isCanvasActive = !1, this._canvas = WMKS.UTIL.createCanvas(!0).getAttribute({
        id: 'mainCanvas',
        tabindex: 1
      }), this._backCanvas = WMKS.UTIL.createCanvas(!0), this._blitTempCanvas = WMKS.UTIL.createCanvas(!0), this.element.classList.add('wmks').append(this._canvas), this.options.enableMP4 && (this._video = WMKS.UTIL.createVideo(!0), this.element.append(this._video));
      var b = function (b) {
      ...
      this.transform = b('transform') || b('WebkitTransform') || b('MozTransform') || b('msTransform') || b('OTransform'), this._vncDecoder = new WMKS.VNCDecoder({
        ...
        onNewDesktopSize: function (b, c) {
          ...
          a._canvas.getAttribute(d).css(e), d.y = c, a._backCanvas.getAttribute(d).css(e), a._blitTempCanvas.getAttribute(d).css(e), a._video && a._video.getAttribute(d).css(e), a._trigger('resolutionchanged', null, d), a.rescaleOrResize(!1)
        },
        onEncodingChanged: function (b) {
          b === 'TightPNG' && !a.isCanvasActive ? (WMKS.LOGGER.info('Activate canvas element since we use TightPNG encoding.'), a.isCanvasActive = !0, a._video && a._video.style.display = 'none', a._canvas.style.display = '') : b === 'MP4' && a.isCanvasActive ? (WMKS.LOGGER.info('Activate video element since we use MP4 encoding.'), a._video ? (a.isCanvasActive = !1, a._canvas.style.display = 'none', a._video.style.display = '') : WMKS.LOGGER.error('Video element doesn't exist.')) : b === 'RawH264' && a.isCanvasActive && WMKS.LOGGER.info('Activate video element since we use raw H264 encoding.')
        },
    ...
    function () {
      'use strict',
        ...
        WMKS.dialogManager.prototype.toggle = function (a) {
          ...
          if (c === d) return;
          if (Date.now() - this.lastToggleTime < this.options.minToggleTime) return;
          d ? this.close() : this.open()
        },
        WMKS.dialogManager.prototype.sendUpdatedState = function (a) {
          this.visible = a, this.lastToggleTime = Date.now(), $.isFunction(this.options.toggleCallback) && this.options.toggleCallback.call(this, [this.options.name, a])
        },
    ...
    function () {
      'use strict',
        ...
        WMKS.extendedKeypad.prototype.create = function () {
          var a = this,
            b = document.querySelector('<div id='ctrlPanePopup'></div>');
          return b.append(this.getControlPaneHtml()), b.dialog({
            ...
            close: function (c) {
              return a._kbManager.cancelModifiers(!0), b.find('.ab-modifier-key.ab-modifier-key-down').classList.remove('ab-modifier-key-down'), a.toggleFunctionKeys(!1), a.sendUpdatedState(!1), !0
            },
            dragStop: function (b) {
        ...
        WMKS.extendedKeypad.prototype.init = function () {
          var a = this,
            b = this.dialog,
            c = function (b) {
              var c = parseInt(document.querySelector(this).getAttribute('abkeycode'), 10);
              return a._kbManager.handleSoftKb(c, !1), !1
            };
          b.find('.ab-modifier-key').on('touchstart', function (b) {
            var c = document.querySelector(this).classList.contains('ab-modifier-key-down'),
              d = parseInt(document.querySelector(this).getAttribute('abkeycode'), 10);
            return isNaN(d) ? (WMKS.LOGGER.debug('Got NaN as modifier key. Skipping it.'), !1) : (document.querySelector(this).classList.toggle('ab-modifier-key-down'), a._kbManager.updateModifiers(d, c), !1)
          }), b.find('#fnMasterKey').removeEventListener('touchstart').on('touchstart', function (b) {
            return a.toggleFunctionKeys(), !1
          }), b.find('.ab-extended-key').removeEventListener('touchstart').on('touchstart', c), b.find('.ab-flip').removeEventListener('touchstart').on('touchstart', function () {
            return document.querySelector(this).parents('.flip-container').classList.toggle('perform-flip'), a.toggleFunctionKeys(!1), !1
          }), b.parentNode.getAttribute('id', 'ctrlPaneWidget'), b.parentNode.parentNode.append(this.getFunctionKeyHtml()), document.querySelector('#fnKeyPad').find('.ab-extended-key').removeEventListener('touchstart').on('touchstart', c), b.parentNode.removeEventListener('orientationchange.ctrlpane').on('orientationchange.ctrlpane', function () {
            a._widget.requestElementReposition(document.querySelector(this)), a.positionFunctionKeys()
          })
        },
        WMKS.dialogManager.prototype.disconnect = function () {
          var a = this.dialog;
          a.find('#fnMasterKey').removeEventListener('touchstart'), a.find('.ab-extended-key').removeEventListener('touchstart'), a.find('.ab-flip').removeEventListener('touchstart'), a.parentNode.removeEventListener('orientationchange.ctrlpane'), document.querySelector('#fnKeyPad').find('.ab-extended-key').removeEventListener('touchstart')
        },
        ...
        WMKS.extendedKeypad.prototype.toggleFunctionKeys = function (a) {
          var b = document.querySelector('#fnKeyPad'),
            c = a || typeof a == 'undefined' && !b.matches(':visible');
          b.toggle(c), document.querySelector('#fnMasterKey').classList.toggle('ab-modifier-key-down', c), this.positionFunctionKeys()
        },
        WMKS.extendedKeypad.prototype.positionFunctionKeys = function () {
          var a = document.querySelector('#fnKeyPad'),
            b = document.querySelector('#ctrlPaneWidget');
          if (a.matches(':visible')) {
            a.position({
              my: 'right bottom',
              at: 'right top',
              of: b,
              collision: 'flip'
            }), document.querySelector('#fnKeyInnerBorder').height(a.getBoundingClientRect().height - 2).width(a.getBoundingClientRect().width - 2);
            var c = a.offset().top + a.getBoundingClientRect().height,
              d = c <= b.offset().top + b.getBoundingClientRect().height;
            this.adjustFunctionKeyStyle(d);
            var e;
            d ? (e = parseInt(b.css('z-index'), 10) - 1, document.querySelector('#fnKeyInnerBorder').css('border-color', '#d5d5d5')) : (e = parseInt(document.querySelector('#ctrlPaneWidget').css('z-index'), 10) + 1, document.querySelector('#fnKeyInnerBorder').css('border-color', '#aaa')), a.css('z-index', e.toString())
          }
          return !0
        },
        WMKS.extendedKeypad.prototype.adjustFunctionKeyStyle = function (a) {
          var b = document.querySelector('#fnKeyPad'),
            c = b.find('.key-group');
          a ? c.classList.contains('down-position') && (c.classList.remove('down-position'), c.classList.add('up-position'), b.classList.remove('fnKey-pane-wrapper-down'), b.classList.add('fnKey-pane-wrapper')) : c.classList.contains('up-position') && (c.classList.remove('up-position'), c.classList.add('down-position'), b.classList.remove('fnKey-pane-wrapper'), b.classList.add('fnKey-pane-wrapper-down'))
        }
    }(),
    function () {
      'use strict',
        ...
        WMKS.trackpadManager.prototype.create = function () {
          var a, b = this;
          return !this._widget || !this._canvas ? (WMKS.LOGGER.debug('Trackpad dialog creation has been aborted. Widget or Canvas is not ready.'), null) : (a = document.querySelector(this.getTrackpadHtml()), a.dialog({
            autoOpen: !1,
            closeOnEscape: !0,
            resizable: !1,
            position: {
            ...
            create: function (a) {
              b.layout(document.querySelector(this).parentNode)
            }
          }), a)
        },
        WMKS.trackpadManager.prototype.init = function () {
          ...
          this._widget.requestElementReposition(a.parentNode, !0), c = a.find('#trackpadSurface').on('touchstart', function (a) {
        ...
        WMKS.trackpadManager.prototype.disconnect = function () {
          var a = this.dialog,
            b, c, d;
          if (!a) return;
          b = a.find('#trackpadSurface').removeEventListener('touchmove').removeEventListener('touchstart').removeEventListener('touchend'), c = a.find('#trackpadLeft').removeEventListener('touchstart').removeEventListener('touchend'), d = a.find('#trackpadRight').removeEventListener('touchstart').removeEventListener('touchend')
        },
        WMKS.trackpadManager.prototype.layout = function (a) {
          var b = this._canvas,
            c, d;
          if (!a || !b) return;
          c = a.parentNode, d = b.parentNode, c !== d && d.append(a)
        },
        WMKS.trackpadManager.prototype.trackpadClickStart = function (a, b) {
          return b !== WMKS.CONST.CLICK.left && b !== WMKS.CONST.CLICK.right ? (WMKS.LOGGER.debug('assert: unknown button ' + b), !1) : (document.querySelector(a.target).classList.add('button-highlight'), this._widget.sendMouseButtonMessage(this.getMousePosition(), !0, b), !1)
        },
        WMKS.trackpadManager.prototype.trackpadClickEnd = function (a, b) {
          return b !== WMKS.CONST.CLICK.left && b !== WMKS.CONST.CLICK.right ? (WMKS.LOGGER.debug('assert: unknown button ' + b), !1) : (document.querySelector(a.target).classList.remove('button-highlight'), this._widget.sendMouseButtonMessage(this.getMousePosition(), !1, b), !1)
        },
  ...
  g.CHUNK_COMMON_HEADER_SIZE = 4, g.CHUNK_LARGE_HEADER_SIZE = 4, g.CHUNK_EXTENSION_HEADER_SIZE = 4, g.CHUNK_MAX_LEN = 65536, g.CHUNK_LARGE_MAX_LEN = 4294966272, g.SESSION_STATE = {
    ...
    this.toggleRelativePad = function (a) {
      ...
      c.css({
        '-webkit-user-select': 'none',
        '-webkit-touch-callout': 'none'
      }), e.cursorIcon = document.querySelector('<div/>').classList.add('feedback-container cursor-icon').append(b), e.clickFeedback = document.querySelector('<div/>').classList.add('feedback-container tap-icon').append(b), e.dragFeedback = document.querySelector('<div/>').classList.add('feedback-container drag-icon').append(b), e.pulseFeedback = document.querySelector('<div/>').classList.add('feedback-container pulse-icon').append(b), e.scrollFeedback = document.querySelector('<div/>').classList.add('feedback-container scroll-icon').append(b)
      ...
      e.cursorIcon && (a ? e.cursorIcon.style.display = '' : e.cursorIcon.style.display = 'none')
    }, this.destroy = function () {
    ...
    function () {
      'use strict',
        ...
        WMKS.relativePadManager.prototype.init = function () {
          ...
          this._widget.requestElementReposition(a.parentNode, !0), c = a.find('#relativePadSurface').on('mousemove', function (a) {
        ...
        WMKS.relativePadManager.prototype.disconnect = function () {
          var a = this.dialog,
            b;
          if (!a) return;
          b = a.find('#relativepadSurface').removeEventListener('mousemove').removeEventListener('mouseup').removeEventListener('mousedown')
        },
        ...
        WMKS.relativePadManager.prototype.relativepadMouseMove = function (a) {
          var b, c, d, e, f = document.querySelector(a.target),
            g = this._widget;
          return this.state === WMKS.CONST.RELATIVEPAD.STATE.idle ? !1 : (b = a.pageX, c = a.pageY, c < f.offset().top || c > f.offset().top + f.getBoundingClientRect().height || b < f.offset().left || b > f.offset().left + f.getBoundingClientRect().width ? (this.state === WMKS.CONST.RELATIVEPAD.STATE.move && (this.state = WMKS.CONST.RELATIVEPAD.STATE.idle, this.history.length = 0), !1) : (this.state === WMKS.CONST.RELATIVEPAD.STATE.move && (d = this.computeNewCursorLocation(b, c), WMKS.VNCDecoder.cursorPosition && (e = this._widget.getRelativeMouseCanvasPosition(WMKS.VNCDecoder.cursorPosition), g._relativeMouseHandler.moveCursor(e.x, e.y)), g.sendMouseMoveMessage(d), this.history.shift(), this.history.push({
            x: b,
            y: c
    ...
    $.extend(WMKS.CONST, {
      ...
      rescaleOrResize: function (a) {
        var b = 1,
          ...
          d = 0,
          e = 'center center',
          f = this.element.getBoundingClientRect().width,
          g = this.element.getBoundingClientRect().height;
        this._canvas.css({
          width: this._guestWidth / this._pixelRatio,
          height: this._guestHeight / this._pixelRatio
        });
        var h = this._canvas.getBoundingClientRect().width,
          i = this._canvas.getBoundingClientRect().height;
      ...
      var h = function (a) {
        ...
        i = function (a) {
          document.querySelector(window).removeEventListener('resize.wmks', h), c.wmks[0].style.cssText = c.oldCssText, c.wmksData.rescaleOrResize(!0), document.querySelector(window).removeEventListener('resize.wmks', i), d(WMKS.CONST.Events.FULL_SCREEN_CHANGE, a, {
            isFullScreen: !1
          })
        };
      ...
      }, document.querySelector(document).on(this.fullScreenChangeEventStr, this.fullScreenChangeHandler), WMKS.CoreWMKS.prototype.enterFullScreen = function () {
        if (WMKS.UTIL.isFullscreenNow() || !WMKS.UTIL.isFullscreenEnabled()) return;
        this.oldCssText = a[0].style.cssText, console.log('old css is ' + this.oldCssText), document.querySelector(window).removeEventListener('resize.wmks', h), document.querySelector(window).on('resize.wmks', h), WMKS.UTIL.toggleFullScreen(!0, a[0])
      }, WMKS.CoreWMKS.prototype.exitFullScreen = function () {
        if (!WMKS.UTIL.isFullscreenNow()) return;
        document.querySelector(window).on('resize.wmks', i), WMKS.UTIL.toggleFullScreen(!1)
      }
      ...
      this.wmksData && (clearTimeout(this.wmksData._vncDecoder.resolutionTimer), this.wmksData.destroy()), document.querySelector(document).removeEventListener(this.fullScreenChangeEventStr, this.fullScreenChangeHandler), document.querySelector(window).removeEventListener('resize.wmks'), this.wmksData = null, this.wmks = null
      ...
      if (!a || !document.querySelector('#' + a)) throw new Error('Invalid parameter');
      var c = document.querySelector('#' + a).nwmks(b),
