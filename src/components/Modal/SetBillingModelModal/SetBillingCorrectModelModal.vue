<!--
  * 파일명 : SetBillingCorrectModelModal.vue
  * 파일 기능 : 빌링 > 모델 > 서버별 보정 모델, 관계사별 보정 모델을 관리할 수 있는 모달
  * 작성자 : 김예담 외 3명
  * 최종 작성일 : 2021-01-29
...
<template>
  <el-dialog
    :title="send === 'server' ? $t('common.MODAL.manageServerModel') : $t('common.MODAL.manageAffModel')"
  ...
  >
    ...
    <section class="modal-body">
      ...
      <div
        v-else-if="!copiedData.length && !newBillingForm"
        class="empty-data"
      >
...
export default {
  name: 'SetBillingCorrectModelModal',
  ...
  methods: {
    ...
    saveBillingGroup () {
      ...
      if (this.newBilling.crud === 'create') { // 과금 모델 생성일 때
        ...
        this.editingBillingModel = this.cloneDeep(this.newBilling)
      }
    ...
    applyUpdateService (rowItem) {
      rowItem.beforeEditItem = this.cloneDeep(rowItem)
  ...
  data () {
    return {
      ...
      columns: [
        { header: this.$t('bill.corrModel'), binding: 'title', width: '*', foldable: true, customHtml: true }
      ],
