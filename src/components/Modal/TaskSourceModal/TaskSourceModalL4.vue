...
import { NetworkL4UpdateCoreInfoStep } from '@sd-fe/cmp-core'
import TaskSourceModalMixins from './TaskSourceModalMixins.script'

export default {
  name: 'TaskSourceModalL4',
  mixins: [TaskSourceModalMixins],
  components: {
    'network-l4-core-info-step': NetworkL4UpdateCoreInfoStep
  },
  computed: {
    ...mapState({
  ...
  methods: {
    ...
    async confirm () {
      ...
      const conditions = [
        ...
        { condition: data.ip, message: this.$v('가상 IP를 입력하세요.') },
