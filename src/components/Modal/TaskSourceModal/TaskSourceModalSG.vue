...
import { NetworkSgUpdateCoreInfoStep } from '@sd-fe/cmp-core'
import TaskSourceModalMixins from './TaskSourceModalMixins.script'

export default {
  name: 'TaskSourceModalSG',
  mixins: [TaskSourceModalMixins],
  components: {
    'network-sg-core-info-step': NetworkSgUpdateCoreInfoStep
  },
  computed: {
    ...mapState({
  ...
  methods: {
    ...
    async confirm () {
      ...
      const conditions = [
        ...
        { condition: data.vrserverName, message: this.$v('로드밸런스명을 입력하세요.') },
        { condition: data.netIdx, message: this.$v('스위치장비명을 입력하세요.') },
        // { condition: data.vrserverName, message: this.$v('가상IP를 입력하세요.') },
